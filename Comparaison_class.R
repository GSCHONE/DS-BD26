...
REStmp=RES
REStmp %>% mutate_if(is.character,as.factor)->REStmp
REStmp %>% select(-elas1se)->REStmp
resultatsGlobauxCLASStmp =data.frame(colnames(REStmp)[2:ncol(REStmp)])
for (k in 2:ncol(REStmp)){
     
     cm<-caret::confusionMatrix(reference=REStmp$Y,data=REStmp[,k])
     resultatsGlobauxCLASStmp[k-1,"Accuracy"]=cm$overall[1]
     
}
conf_mat(REStmp,truth=Y,estimate=elasmin)
conf_mat(REStmp,truth=Y,estimate=xgboost)
conf_mat(REStmp,truth=Y,estimate=elas1se)
conf_mat(REStmp,truth=Y,estimate=kknn)
conf_mat(REStmp,truth=Y,estimate=mlp)
conf_mat(REStmp,truth=Y,estimate=foret)
